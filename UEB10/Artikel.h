...
namespace artikel {
	 ...
	 */
	class ArtikelException : public std::logic_error {
	public:
		ArtikelException(const std::string& msg = "") : logic_error(msg) {}
	};
	...
	public:
		 ...
		 */
		Artikel(int artikelNr, std::string bezeichnung, double artikelPreis, int bestand = 0)
		 ...
		 */
		friend std::ostream& operator<<(std::ostream&, const Artikel&);
	private:
		int artikelNr; ///< Artikel Nummer
		std::string bezeichnung; ///< Artikel Bezeichnung
